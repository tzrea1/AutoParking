...
class SequentialMemory(Memory):
    ...
    # TODO : 把for去掉，增加效率
    def sample_2(self, batch_size, batch_idxs=None):
        if batch_idxs is None:
            # Draw random indexes such that we have at least a single entry before each
            # index.
            batch_idxs = sample_batch_indexes(0, self.nb_entries - 1, size=batch_size)
        ...
        for idx in batch_idxs:
            ...
            state1 = []
            state1.append(self.next_observations[idx])
    ...
    def sample_and_split(self, batch_size, batch_idxs=None):
        experiences = self.sample_2(batch_size, batch_idxs)
    ...
    def append(self, observation, action, reward, terminal, next_observation, training=True):
